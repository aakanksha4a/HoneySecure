...
//             // This is where you would construct the path of the file you want to attach.
//            if (honeypotField == null || honeypotField.isEmpty()) {
//                // Handle error: File path is invalid
            ...
            if (attempts >= MAX_ATTEMPTS) {
               ...
               new Thread(() -> {
                    try {
                       ...
                       String logFilePath = getServletContext().getRealPath("/") + "honeypot_logs/attempt_log.txt";
                       try {
                           File logDir = new File(getServletContext().getRealPath("/") + "honeypot_logs");
                           if (!logDir.exists()) {
                               logDir.mkdir();
                           }
                           
                           java.io.FileWriter fw = new java.io.FileWriter(logFilePath);
                           fw.write("Failed login attempts detected for user: " + username + 
                                    "\nAttempt time: " + new java.util.Date() + 
                                    "\nPassword attempted: " + password);
                           fw.close();
                           
                           // Now send email with the log file we just created
                           MailUtil.sendFakeUserAlertWithAttachment(username, logFilePath);
                       } catch (Exception e) {
                           System.out.println("Error creating log file: " + e.getMessage());
                           e.printStackTrace();
                           
                           // Try sending email without attachment as fallback
                           MailUtil.sendFakeUserAlertWithAttachment(username, null);
                       }
