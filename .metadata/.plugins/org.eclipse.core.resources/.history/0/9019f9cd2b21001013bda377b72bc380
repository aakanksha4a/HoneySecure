...
public class newUser extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(hd.addNewUser(h)) {
			request.setAttribute("msg", "Added Successfully..can now go back to login page");
			
		}
		else {
			request.setAttribute("msg", "User could not be Added..try with a different username");
		}
